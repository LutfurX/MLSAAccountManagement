...
interface MonthlyExpensePrintModalProps {
  isOpen: boolean;
  onClose: () => void;
  monthKey: string;
...
export const MonthlyExpensePrintModal: React.FC<MonthlyExpensePrintModalProps> = ({
  isOpen,
  onClose,
  monthKey,
  expenses,
    ...
    >
      <style>{`
        @media print {
          ...
          #monthly-expense-print-area, #monthly-expense-print-area * {
            visibility: visible !important;
          }
          #monthly-expense-print-area {
      ...
      >
        ...
        <div className="no-print flex items-center justify-between p-4 border-b border-slate-200 bg-slate-50 shrink-0">
          <div className="flex items-center space-x-2.5">
            ...
            <div>
              <h3 className="font-bold text-slate-900 text-base">
                {monthLabel} মাসের খরচের রিপোর্ট ও প্রিন্ট
              </h3>
          ...
          <div className="flex items-center space-x-2">
            <button
              onClick={handlePrint}
              className="px-4 py-2 bg-rose-600 hover:bg-rose-700 text-white font-bold text-sm rounded-lg shadow-xs hover:shadow transition-all flex items-center gap-2 cursor-pointer"
            >
            ...
            <div>
              <div className="text-center border-b-2 border-slate-800 pb-4 mb-5">
                ...
                <div className="inline-block mt-2 px-4 py-1 bg-slate-100 border border-slate-300 rounded-full text-xs font-bold text-slate-800 uppercase tracking-wide">
                  মাসিক ব্যয়ের বিবরণী ও ভাউচার রেজিস্টার
                </div>
              </div>
              ...
              <div className="flex items-center justify-between text-xs text-slate-700 mb-4 pb-2 border-b border-slate-200">
                <div>
                  ...
                  <span className="text-rose-800 font-extrabold text-sm">{monthLabel}</span>
                </div>
                <div>
                ...
                <div>
                  <span className="font-bold text-slate-900">মোট ভাউচার সংখ্যা: </span>
                  <span>{toBanglaDigits(sortedExpenses.length)} টি</span>
                </div>
              </div>
                ...
                <table className="w-full text-left text-xs border-collapse border border-slate-300">
                  <thead className="bg-slate-100 text-slate-900 font-bold">
                    <tr>
                      ...
                      <th className="border border-slate-300 py-2.5 px-3">ক্যাটাগরি</th>
                      <th className="border border-slate-300 py-2.5 px-3">খরচের বিবরণ</th>
                      <th className="border border-slate-300 py-2.5 px-3 text-center">ভাউচার</th>
                  ...
                  <tbody>
                    {sortedExpenses.length === 0 ? (
                      ...
                      sortedExpenses.map((exp, idx) => (
                        <tr key={exp.id} className="even:bg-slate-50">
                          ...
                          <td className="border border-slate-300 py-2 px-3">
                            ...
                            {exp.notes && (
                              <span className="block text-[11px] text-slate-500">{exp.notes}</span>
                            )}
                          </td>
                          <td className="border border-slate-300 py-2 px-3 text-center text-[11px]">
                            {exp.voucherImage ? (
                              <span className="text-emerald-700 font-bold">সংযুক্ত</span>
                            ) : (
                  ...
                  <tfoot>
                    <tr className="bg-slate-100 font-black text-slate-950">
                      <td colSpan={5} className="border border-slate-300 py-2.5 px-3 text-right text-xs uppercase tracking-wider">
                        {monthLabel} মাসের সর্বমোট ব্যয়:
                      </td>
                      <td className="border border-slate-300 py-2.5 px-3 text-right text-sm text-rose-800 whitespace-nowrap font-black">
              ...
              <div className="grid grid-cols-3 gap-6 text-center text-xs">
                <div>
                  <div className="border-t border-slate-500 pt-1 font-bold text-slate-800">
                    হিসাবরক্ষক / ক্যাশিয়ার
                  </div>
                  <span className="text-[10px] text-slate-500">প্রস্তুতকারীর স্বাক্ষর</span>
                </div>
                <div>
                  <div className="border-t border-slate-500 pt-1 font-bold text-slate-800">
                    অর্থ সম্পাদক / অডিটর
                  </div>
                  <span className="text-[10px] text-slate-500">যাচাই ও পরীক্ষা</span>
                </div>
                <div>
                  <div className="border-t border-slate-500 pt-1 font-bold text-slate-800">
                    অধ্যক্ষ / সভাপতি
                  </div>
                  <span className="text-[10px] text-slate-500">চূড়ান্ত অনুমোদন ও সিল</span>
                </div>
              </div>
            </div>
          </div>
